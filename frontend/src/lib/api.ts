...
export function toApiError(err: unknown): ApiError {
  ...
  if (err instanceof AxiosError && err.response) {
    ...
    const detail =
      (data?.error as Record<string, string>)?.message || (data?.detail as string) || err.message;
      ...
      try {
        if (!refreshPromise) {
          refreshPromise = axios
            .post(`${API_BASE_URL}/api/v1/auth/refresh`, undefined, { withCredentials: true })
            .then(() => undefined)
            .finally(() => {
              refreshPromise = null;
            });
...
export const requirementsApi = {
  ...
  review: (id: string, approved: boolean) => api.post(`/requirements/${id}/review`, { approved }),
};
...
export const customerProfilesApi = {
  ...
  update: (id: string, data: Record<string, unknown>) =>
    api.patch(`/customer-profiles/${id}`, data),
};
...
export const complianceApi = {
  ...
  assessImpact: (mappingId: string) => api.post(`/compliance/assess/${mappingId}`),
...
export const auditApi = {
  ...
  updateAction: (id: string, data: Record<string, unknown>) =>
    api.patch(`/audit/actions/${id}`, data),
};
...
export const testingApi = {
  ...
  validateTests: (suiteId: string) => api.post(`/testing/validate/${suiteId}`),
};
...
export const architectureAdvisorApi = {
  ...
  listPatterns: () => api.get('/architecture-advisor/patterns'),
  getScore: (repo: string) => api.get(`/architecture-advisor/score/${repo}`),
};
...
export const costCalculatorApi = {
  ...
  calculateROI: (data: { regulation: string }) => api.post('/cost-calculator/roi', data),
  getHistory: () => api.get('/cost-calculator/history'),
};

// Evidence Vault API
export const evidenceVaultApi = {
  storeEvidence: (data: Record<string, unknown>) => api.post('/evidence-vault/evidence', data),
  getEvidence: (params?: { framework?: string }) => api.get('/evidence-vault/evidence', { params }),
  verifyChain: (framework: string) => api.get(`/evidence-vault/verify/${framework}`),
  getControlMappings: (framework: string) => api.get(`/evidence-vault/controls/${framework}`),
  createAuditorSession: (data: { auditor_email: string; auditor_name: string }) =>
    api.post('/evidence-vault/auditor-sessions', data),
  generateReport: (framework: string) => api.get(`/evidence-vault/report/${framework}`),
};

// Federated Intelligence API
export const federatedIntelApi = {
  getNetworkStats: () => api.get('/federated-intel/network'),
  getThreats: (params?: { category?: string }) => api.get('/federated-intel/threats', { params }),
  getPatterns: () => api.get('/federated-intel/patterns'),
  getBenchmarks: () => api.get('/federated-intel/benchmarks'),
  getThreatFeed: () => api.get('/federated-intel/feed'),
};

// Marketplace App API
export const marketplaceAppApi = {
  getListingInfo: () => api.get('/marketplace-app/listing'),
  getInstallations: () => api.get('/marketplace-app/installations'),
  handleInstallation: (data: Record<string, unknown>) => api.post('/marketplace-app/install', data),
};

// Industry Packs API
export const industryPacksApi = {
  listPacks: () => api.get('/industry-packs'),
  getPack: (vertical: string) => api.get(`/industry-packs/${vertical}`),
  provisionPack: (vertical: string) => api.post(`/industry-packs/${vertical}/provision`),
};
...
export const nlQueryApi = {
  ...
  getHistory: (limit = 20) => api.get('/nl-query/history', { params: { limit } }),
  submitFeedback: (data: { query_id: string; helpful: boolean }) =>
    api.post('/nl-query/feedback', data),
  getIntents: () => api.get('/nl-query/intents'),
};
...
export const multiLlmApi = {
  ...
  getProviders: () => api.get('/multi-llm/providers'),
  addProvider: (data: { provider: string; model_name: string; weight?: number }) =>
    api.post('/multi-llm/providers', data),
  removeProvider: (provider: string) => api.delete(`/multi-llm/providers/${provider}`),
  getConfig: () => api.get('/multi-llm/config'),
};
...
export const impactTimelineApi = {
  ...
  addEvent: (data: Record<string, unknown>) => api.post('/impact-timeline/events', data),
  generateTasks: (eventId: string) => api.post(`/impact-timeline/events/${eventId}/tasks`),
  getTasks: (params?: { task_status?: string }) => api.get('/impact-timeline/tasks', { params }),
};

// Audit Autopilot API
export const auditAutopilotApi = {
  runGapAnalysis: (framework: string) => api.post(`/audit-autopilot/gap-analysis/${framework}`),
  ...
  listFrameworks: () => api.get('/audit-autopilot/frameworks'),
};
...
export const policySdkApi = {
  ...
  createPolicy: (data: Record<string, unknown>) => api.post('/policy-sdk/policies', data),
  validatePolicy: (policyId: string) => api.post(`/policy-sdk/policies/${policyId}/validate`),
  ...
  listSDKs: () => api.get('/policy-sdk/sdks'),
};

// Compliance Sandbox API
export const complianceSandboxApi = {
  listScenarios: () => api.get('/compliance-sandbox/scenarios'),
  getScenario: (id: string) => api.get(`/compliance-sandbox/scenarios/${id}`),
  createEnvironment: (scenarioId: string) =>
    api.post(`/compliance-sandbox/scenarios/${scenarioId}/start`),
  getEnvironment: (envId: string) => api.get(`/compliance-sandbox/environments/${envId}`),
};
...
export const ideAgentApi = {
  ...
  getFeedbackStats: () => api.get('/ide-agent/feedback/stats'),
};
...
export const impactSimulatorApi = {
  ...
  listScenarios: () => api.get('/impact-simulator/scenarios'),
  getBlastRadius: (scenarioId: string) => api.get(`/impact-simulator/blast-radius/${scenarioId}`),
...
export const marketplaceEnhancedApi = {
  ...
  checkPlanEnforcement: (installationId: string, action?: string) =>
    api.get(`/marketplace-app/installations/${installationId}/plan-check`, {
...
export const evidenceVaultEnhancedApi = {
  getCoverage: (framework: string) => api.get(`/evidence-vault/coverage/${framework}`),
  verifyChainEnhanced: (framework: string) =>
    api.get(`/evidence-vault/verify-enhanced/${framework}`),
  getGaps: (framework: string) => api.get(`/evidence-vault/gaps/${framework}`),
};

// Enhanced Multi-LLM API
export const multiLlmEnhancedApi = {
  analyzeDivergence: (consensusId: string) => api.get(`/multi-llm/divergence/${consensusId}`),
  getProviderHealth: () => api.get('/multi-llm/provider-health'),
};
...
export const selfHostedEnhancedApi = {
  ...
  listOfflineBundles: () => api.get('/self-hosted/offline-bundles'),
  installBundle: (bundleId: string) => api.post(`/self-hosted/offline-bundles/${bundleId}/install`),
  getAirGapStatus: () => api.get('/self-hosted/air-gap-status'),
};

// Enhanced Industry Packs API
export const industryPacksEnhancedApi = {
  getWizardSteps: (vertical: string) => api.get(`/industry-packs/${vertical}/wizard`),
...
export const crossBorderTransferApi = {
  ...
  listSCCs: () => api.get('/cross-border-transfer/scc'),
  runTIA: (flowId: string) => api.post(`/cross-border-transfer/tia/${flowId}`),
  getAdequacyDecisions: () => api.get('/cross-border-transfer/adequacy'),
  getAlerts: () => api.get('/cross-border-transfer/alerts'),
  getReport: () => api.get('/cross-border-transfer/report'),
  getJurisdictions: () => api.get('/cross-border-transfer/jurisdictions'),
};
...
export const stressTestingApi = {
  ...
  listScenarios: () => api.get('/stress-testing/scenarios'),
  runSimulation: (data: { scenario_id: string; iterations?: number }) =>
    api.post('/stress-testing/simulate', data),
  getSimulation: (runId: string) => api.get(`/stress-testing/simulations/${runId}`),
  getReport: () => api.get('/stress-testing/report'),
};

// Zero-Trust Scanner API
export const zeroTrustScannerApi = {
  scan: (data: { repo: string; scan_type?: string }) => api.post('/zero-trust-scanner/scan', data),
  listPolicies: () => api.get('/zero-trust-scanner/policies'),
  ...
  getSummary: () => api.get('/zero-trust-scanner/summary'),
};
...
export const complianceTrainingApi = {
  ...
  getProfile: (developerId: string) => api.get(`/compliance-training/profile/${developerId}`),
  ...
  getTeamReport: (team: string) => api.get(`/compliance-training/team-report/${team}`),
  getLeaderboard: () => api.get('/compliance-training/leaderboard'),
};
...
export const aiObservatoryApi = {
  ...
  classifyRisk: (modelId: string) => api.post(`/ai-observatory/models/${modelId}/classify`),
  ...
  assessCompliance: (modelId: string) => api.post(`/ai-observatory/models/${modelId}/assess`),
  getDashboard: () => api.get('/ai-observatory/dashboard'),
};
...
export const regulationTestGenApi = {
  ...
  getCoverage: (regulation: string) => api.get(`/regulation-test-gen/coverage/${regulation}`),
  runTests: (suiteId: string) => api.post(`/regulation-test-gen/suites/${suiteId}/run`),
  listCoverages: () => api.get('/regulation-test-gen/coverage'),
};
...
export const sentimentAnalyzerApi = {
  ...
  getHeatmap: () => api.get('/sentiment-analyzer/heatmap'),
  getPrioritization: () => api.get('/sentiment-analyzer/prioritization'),
  getReport: () => api.get('/sentiment-analyzer/report'),
};
...
export const incidentPlaybookApi = {
  ...
  getPlaybook: (playbookId: string) => api.get(`/incident-playbook/playbooks/${playbookId}`),
  createIncident: (data: {
    playbook_id: string;
    severity: string;
    title: string;
...
export const costAttributionApi = {
  ...
  getRegulationSummary: (regulation: string) => api.get(`/cost-attribution/summary/${regulation}`),
  analyzeROI: (regulation: string) => api.post(`/cost-attribution/roi/${regulation}`),
  getDashboard: () => api.get('/cost-attribution/dashboard'),
  listSummaries: () => api.get('/cost-attribution/summaries'),
};
...
export const blockchainAuditApi = {
  ...
  getChain: () => api.get('/blockchain-audit/chain'),
  verifyChain: () => api.get('/blockchain-audit/verify'),
  getState: () => api.get('/blockchain-audit/state'),
  ...
  listSmartContracts: () => api.get('/blockchain-audit/smart-contracts'),
};
...
export const marketplaceV2Api = {
  getAppManifest: () => api.get('/marketplace-app/app-manifest'),
  verifyWebhook: (data: { payload: string; signature: string }) =>
    api.post('/marketplace-app/webhook/verify', data),
};

export const chatV2Api = {
  diagnoseRag: (query: string) => api.post('/chat/rag/diagnose', null, { params: { query } }),
...
export const digitalTwinV2Api = {
  ...
  timeTravel: (target_time: string) => api.post('/digital-twin/live/time-travel', { target_time }),
  getTimeline: (days?: number) => api.get('/digital-twin/live/timeline', { params: { days } }),
  blastRadius: (regulation: string, scenario_description?: string) =>
    api.post('/digital-twin/live/blast-radius', null, {
      params: { regulation, scenario_description },
    }),
};
...
export const remediationV2Api = {
  ...
  getTemplate: (id: string) => api.get(`/remediation/templates/${id}`),
  applyTemplate: (data: {
    template_id: string;
...
export const multiLlmV2Api = {
  smartRoute: (data: { text: string; framework?: string }) => api.post('/multi-llm/route', data),
  classifyComplexity: (text: string) =>
    api.post('/multi-llm/classify-complexity', null, { params: { text } }),
};

export const evidenceVaultV2Api = {
  listControls: (framework: string) => api.get(`/evidence-vault/v2/controls/${framework}`),
...
export const selfHostedV2Api = {
  ...
  listBundles: () => api.get('/self-hosted/v2/bundles'),
  createDeployConfig: (data: {
    mode: string;
    license_key?: string;
    llm_backend?: string;
    bundles?: string[];
...
export const industryPacksV2Api = {
  listStarterPacks: () => api.get('/industry-packs/v2/starter-packs'),
  getStarterPack: (id: string) => api.get(`/industry-packs/v2/starter-packs/${id}`),
  provision: (data: { pack_id: string }) => api.post('/industry-packs/v2/provision', data),
...
export const federatedIntelV2Api = {
  ...
  getPrivacyBudget: () => api.get('/federated-intel/v2/privacy-budget'),
  noiseDemo: (params: { true_value: number; epsilon?: number; sensitivity?: number }) =>
    api.post('/federated-intel/v2/noise-demo', null, { params }),
};

// ═══════════════════════════════════════════════════════════════
// Next-Gen v4 API Clients (39 dashboard wiring)
// ═══════════════════════════════════════════════════════════════

export const apiGatewayApi = {
  listClients: (active_only?: boolean) =>
    api.get('/api-gateway/clients', { params: active_only ? { active_only } : {} }),
  getStats: () => api.get('/api-gateway/stats'),
};

export const apiMonetizationApi = {
  listApis: (regulation?: string) =>
    api.get('/api-monetization/apis', { params: regulation ? { regulation } : {} }),
  getRevenue: () => api.get('/api-monetization/revenue'),
};

export const agentSwarmApi = {
  listSessions: () => api.get('/agent-swarm/sessions'),
  getStats: () => api.get('/agent-swarm/stats'),
};

export const agentsMarketplaceApi = {
  searchAgents: (params?: { query?: string; category?: string }) =>
    api.get('/agents-marketplace/agents', { params }),
  getStats: () => api.get('/agents-marketplace/stats'),
};

export const archAdvisorApi = {
  getStats: () => api.get('/arch-advisor/stats'),
  listFrameworks: () => api.get('/arch-advisor/frameworks'),
};

export const auditWorkspaceApi = {
  listWorkspaces: (org_id?: string) =>
    api.get('/audit-workspace/workspaces', { params: { org_id: org_id ?? 'default' } }),
};
...
export const autonomousOsApi = {
  listEvents: () => api.get('/autonomous-os/events'),
  getStats: () => api.get('/autonomous-os/stats'),
  getHealth: () => api.get('/autonomous-os/health'),
};

export const autoRemediationApi = {
  listPipelines: (params?: { repo?: string; pipeline_status?: string }) =>
    api.get('/auto-remediation/pipelines', { params }),
  getStats: () => api.get('/auto-remediation/stats'),
};

export const boardReportsApi = {
  getExecutiveSummary: (params?: { org_id?: string; period?: string }) =>
    api.get('/board-reports/executive-summary', { params }),
};

export const certAutopilotApi = {
  listJourneys: () => api.get('/cert-autopilot/journeys'),
};

export const certPipelineApi = {
  listRuns: (params?: { framework?: string; repo?: string }) =>
    api.get('/cert-pipeline/runs', { params }),
  getStats: () => api.get('/cert-pipeline/stats'),
};

export const chaosEngineeringApi = {
  listExperiments: (status?: string) =>
    api.get('/chaos-engineering/experiments', { params: status ? { status } : {} }),
  getStats: () => api.get('/chaos-engineering/stats'),
};

export const cicdRuntimeApi = {
  listChecks: (deployment_id?: string) =>
    api.get('/cicd-runtime/checks', { params: deployment_id ? { deployment_id } : {} }),
  getStats: () => api.get('/cicd-runtime/stats'),
};

export const clientSdkApi = {
  listPackages: () => api.get('/client-sdk/packages'),
  getStats: () => api.get('/client-sdk/stats'),
};

export const codeReviewAgentApi = {
  listReviews: (params?: { repo?: string; limit?: number }) =>
    api.get('/code-review-agent/reviews', { params }),
  getStats: () => api.get('/code-review-agent/stats'),
};

export const complianceApiStandardApi = {
  listVersions: () => api.get('/compliance-api-standard/versions'),
  getStats: () => api.get('/compliance-api-standard/stats'),
};

export const complianceBadgeApi = {
  getStats: () => api.get('/compliance-badge/badge/default'),
};

export const complianceCloningApi = {
  listReferenceRepos: (params?: { industry?: string; language?: string }) =>
    api.get('/compliance-cloning/reference-repos', { params }),
};

export const complianceDataLakeApi = {
  getStats: () => api.get('/compliance-data-lake/stats'),
};

export const complianceDebtApi = {
  listItems: (framework?: string) =>
    api.get('/compliance-debt/items', { params: framework ? { framework } : {} }),
  getStats: () => api.get('/compliance-debt/stats'),
};

export const complianceEditorApi = {
  getStats: () => api.get('/compliance-editor/stats'),
};

export const complianceExportApi = {
  listExports: (limit?: number) =>
    api.get('/compliance-export/exports', { params: { limit: limit ?? 20 } }),
  getSummary: () => api.get('/compliance-export/summary'),
};

export const complianceObservabilityApi = {
  listMetrics: () => api.get('/compliance-observability/metrics'),
  getStats: () => api.get('/compliance-observability/stats'),
};

export const complianceSdkPackagesApi = {
  listSdks: (language?: string) =>
    api.get('/compliance-sdk/sdks', { params: language ? { language } : {} }),
  getUsage: () => api.get('/compliance-sdk/usage'),
};

export const costEngineApi = {
  listAttributions: (params?: { team?: string; framework?: string }) =>
    api.get('/cost-engine/attributions', { params }),
  getRoi: () => api.get('/cost-engine/roi'),
};

export const crossCloudMeshApi = {
  listAccounts: () => api.get('/cross-cloud-mesh/accounts'),
  getPosture: () => api.get('/cross-cloud-mesh/posture'),
  getStats: () => api.get('/cross-cloud-mesh/stats'),
};

export const crossOrgBenchmarkApi = {
  getStats: () => api.get('/cross-org-benchmark/stats'),
  listIndustries: () => api.get('/cross-org-benchmark/industries'),
};

export const crossRepoGraphApi = {
  getGraph: () => api.get('/cross-repo-graph/graph'),
  getOrgScore: () => api.get('/cross-repo-graph/score'),
};

export const dataMeshFederationApi = {
  listNodes: () => api.get('/data-mesh-federation/nodes'),
  getStats: () => api.get('/data-mesh-federation/stats'),
};

export const entityRollupApi = {
  getHierarchy: () => api.get('/entity-rollup/hierarchy'),
};

export const gameEngineApi = {
  listScenarios: (params?: { category?: string; difficulty?: string }) =>
    api.get('/game-engine/scenarios', { params }),
  getLeaderboard: (limit?: number) =>
    api.get('/game-engine/leaderboard', { params: { limit: limit ?? 10 } }),
};

export const horizonScannerApi = {
  getTimeline: (params?: { jurisdiction?: string; framework?: string; months_ahead?: number }) =>
    api.get('/horizon-scanner/timeline', { params }),
};

export const policyMarketplaceApi = {
  listPacks: (params?: { sort?: string; limit?: number }) =>
    api.get('/policy-marketplace/packs', { params: { sort: 'popular', limit: 20, ...params } }),
  getStats: () => api.get('/policy-marketplace/stats'),
};

export const regulationDiffApi = {
  listVersions: (regulation?: string) =>
    api.get('/regulation-diff/versions', { params: regulation ? { regulation } : {} }),
  listDiffs: (regulation?: string) =>
    api.get('/regulation-diff/diffs', { params: regulation ? { regulation } : {} }),
};

export const complianceCopilotApi = {
  listViolations: (params?: { framework?: string; severity?: string; limit?: number }) =>
    api.get('/compliance-copilot/violations', { params }),
};

export const iacPolicyApi = {
  listRules: (params?: { provider?: string; framework?: string }) =>
    api.get('/iac-policy/rules', { params }),
  getScanHistory: () => api.get('/iac-policy/scan-history'),
};

export const knowledgeGraphApi = {
  getNodeTypes: () => api.get('/knowledge-graph/node-types'),
  getQueryTemplates: () => api.get('/knowledge-graph/templates'),
};

export const pairProgrammingApi = {
  getContext: (language: string) => api.get(`/pair-programming/context/${language}`),
  getRules: () => api.get('/pair-programming/rules'),
};
