...
function NoDataIllustration() {
  return (
    <svg className="w-32 h-32 text-gray-300 dark:text-gray-600" viewBox="0 0 128 128" fill="none">
      ...
      <path
        d="M64 82v12M64 82l4 4M64 82l-4 4"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
      />
...
function AccessDeniedIllustration() {
  return (
    <svg
      className="w-32 h-32 text-yellow-300 dark:text-yellow-600"
      viewBox="0 0 128 128"
      fill="none"
    >
...
> = {
  ...
  'no-data': {
    ...
    actions: [{ label: 'Get started', variant: 'primary', icon: <Plus className="h-4 w-4" /> }],
  },
  'no-repositories': {
    ...
    actions: [
      ...
      {
        label: 'Learn more',
        href: '/docs/guides/connecting-repositories',
        variant: 'secondary',
    ...
    actions: [
      {
        ...
        href: '/dashboard/regulations/add',
        variant: 'primary',
        icon: <Plus className="h-4 w-4" />,
      },
...
export function EmptyState({
  ...
  return (
    ...
    >
      {/* Illustration */}
      <div className="mb-6">{icon || config.illustration}</div>

      {/* Content */}
      <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">{displayTitle}</h3>
      <p className="text-gray-500 dark:text-gray-400 max-w-md mb-6">{displayDescription}</p>
      ...
      {displayActions.length > 0 && (
        <div className="flex items-center gap-3 flex-wrap justify-center">
          {displayActions.map((action, index) => {
            ...
            return (
              <button type="button" key={index} onClick={action.onClick} className={buttonClasses}>
                {action.icon}
                {action.label}
              </button>
            );
