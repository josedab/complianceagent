...
import type { PolicyValidation } from '@/types/nextgen'
...
export default function PolicySDKDashboard() {
  ...
  const { data: policies, loading: policiesLoading, error: policiesError, refetch: refetchPolicies } = usePolicies()
  const { data: marketplace, loading: mktLoading, error: mktError, refetch: refetchMkt } = usePolicyMarketplace()
  const { data: sdks, loading: sdksLoading, error: sdksError, refetch: refetchSdks } = useSDKInfo()
  const { mutate: validate, loading: validating } = useValidatePolicy()

  const loading = policiesLoading || mktLoading || sdksLoading
  const error = policiesError || mktError || sdksError
  ...
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        <p className="text-red-800">Error loading Policy SDK: {error.message}</p>
        <button onClick={() => { refetchPolicies(); refetchMkt(); refetchSdks(); }} className="mt-2 px-3 py-1 bg-red-100 text-red-700 rounded text-sm hover:bg-red-200">Retry</button>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stats */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard icon={<Shield className="h-5 w-5 text-blue-600" />} title="Total Policies" value={policyList.length.toString()} subtitle="Active policies" />
        <StatCard icon={<Package className="h-5 w-5 text-purple-600" />} title="Marketplace" value={mktList.length.toString()} subtitle="Community policies" />
        <StatCard icon={<Terminal className="h-5 w-5 text-green-600" />} title="SDK Languages" value={sdkList.length.toString()} subtitle="Python, TS, Go" />
        <StatCard icon={<Download className="h-5 w-5 text-orange-600" />} title="Total Installs" value={mktList.reduce((s, m) => s + m.installs, 0).toLocaleString()} subtitle="Across marketplace" />
      </div>
      ...
      {tab === 'policies' && (
        <div className="space-y-4">
          ...
          <div className="card">
            ...
            {policyList.length === 0 ? (
              <p className="text-gray-500 text-sm p-4">No policies available yet.</p>
            ) : (
      ...
      {tab === 'marketplace' && (
        <div className="card">
          ...
          {mktList.length === 0 ? (
            <p className="text-gray-500 text-sm p-4">No marketplace entries yet.</p>
          ) : (
            <div className="divide-y divide-gray-100">
              {mktList.map((m) => (
      ...
      {tab === 'sdks' && (
        <div className="space-y-4">
          {sdkList.length === 0 ? (
            <p className="text-gray-500 text-sm">No SDK packages available.</p>
          ) : (
            sdkList.map((sdk) => (
