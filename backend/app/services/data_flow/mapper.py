...
class DataFlowMapper:
                ...
                for dest in locations[i + 1 :]:
                    if src.country_code != dest.country_code:
                        flow = DataFlow(
                            ...
                            data_types=list(set(src.data_types) & set(dest.data_types))
                            or src.data_types,
    ...
    def _extract_region(self, content: str, infra_type: str) -> str | None:
        ...
        azure_region = re.search(
            r'["\']?(east|west|central|north|south)[a-z]*["\']?', content, re.IGNORECASE
        )
        if azure_region:
            return azure_region.group().strip("'\"").lower()

        # GCP region patterns
        gcp_region = re.search(
            r'["\']?(us|europe|asia|australia|southamerica)-[a-z]+\d["\']?', content
        )
        if gcp_region:
    ...
    def _assess_compliance(self, flow: DataFlow) -> ComplianceStatus:
        ...
        if flow.transfer_mechanism == TransferMechanism.NONE:
            if (
                ...
                and flow.destination_country not in EEA_COUNTRIES
            ):
    ...
    def _get_required_actions(self, flow: DataFlow) -> list[str]:
        ...
        if flow.destination_country in DATA_LOCALIZATION_COUNTRIES:
            actions.append(
                f"Review data localization requirements: {DATA_LOCALIZATION_COUNTRIES.get(flow.destination_country)}"
            )
    ...
    def _calculate_statistics(self, flow_map: DataFlowMap) -> None:
        ...
        flow_map.compliant_flows = len(
            [f for f in flow_map.flows if f.compliance_status == ComplianceStatus.COMPLIANT]
        )
        ...
        flow_map.action_required_flows = len(
            [
                ...
                if f.compliance_status
                in [ComplianceStatus.ACTION_REQUIRED, ComplianceStatus.NON_COMPLIANT]
            ]
        )

        flow_map.critical_risks = len(
            [f for f in flow_map.flows if f.risk_level == RiskLevel.CRITICAL]
        )
        ...
        flow_map.countries_involved = list({l.country_code for l in flow_map.locations})
        flow_map.regions_involved = list({l.region for l in flow_map.locations if l.region})
