...
Return JSON only."""
        ...
        try:
            async with self.copilot_client:
                ...
                result = self.copilot_client._parse_json_response(
                    response.content, "fix generation"
                )
                ...
                if isinstance(result, dict) and result.get("fixed_code"):
                    ...
                    return AutoFix(
                        ...
                        description=result.get(
                            "explanation", f"AI-generated fix for {violation.code}"
                        ),
                        confidence=0.85,
                        status=AutoFixStatus.GENERATED,
        ...
        async with GitHubClient(access_token=access_token) as client:
            try:
                ...
                if fix.original_code in current_content:
                    ...
                    logger.warning("Original code not found in file, cannot apply fix")
                    return False
