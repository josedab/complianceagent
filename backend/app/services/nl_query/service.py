...
class NLQueryService:
    ...
    async def query(self, text: str, context: dict | None = None) -> QueryResult:
        ...
        logger.info(
            "Query processed", intent=intent.value, sources=len(sources), time_ms=round(elapsed, 1)
        )
        return result
    ...
    def _handle_regulation_lookup(self, text: str) -> tuple[str, list[QuerySource]]:
        ...
        for key, reg in _REGULATION_KB.items():
            if key in text_lower or reg["name"].lower() in text_lower:
                for art, desc in reg["key_articles"].items():
                    if any(
                        kw in text_lower
                        for kw in [art.lower(), desc.split("-")[0].strip().lower(), key]
                    ):
                        sources.append(
                            QuerySource(
        ...
        if not answers:
            return (
                "No matching regulations found. Try specifying a framework (GDPR, HIPAA, PCI-DSS, SOC 2).",
                [],
            )
    ...
    def _handle_violation_query(self, text: str) -> tuple[str, list[QuerySource]]:
        ...
        return (
            ...
            "- 1 Minor: Audit logging incomplete for admin actions (SOC 2 CC7.2)"
        ), sources
    ...
    def _handle_code_search(self, text: str) -> tuple[str, list[QuerySource], list[CodeReference]]:
        sources = [
            QuerySource(
                ...
                snippet="Found relevant code references",
            )
        ]
        code_refs = [
            CodeReference(
    ...
    def _handle_audit_query(self, text: str) -> tuple[str, list[QuerySource]]:
        sources = [
            QuerySource(
                ...
                snippet="Current audit readiness: 72%",
            )
        ]
        return (
            ...
            "Run the Audit Autopilot for a detailed gap analysis."
        ), sources
    ...
    def _handle_status_check(self, text: str) -> tuple[str, list[QuerySource]]:
        sources = [
            QuerySource(
                ...
                reference="dashboard",
                relevance_score=0.9,
                snippet="Overall compliance score: 85%",
            )
        ]
        return (
            ...
            "- Drift score: 3.2% (within threshold)"
        ), sources
    ...
    def _handle_comparison(self, text: str) -> tuple[str, list[QuerySource]]:
        ...
        return (
            ...
            "- SOC 2 focuses on service organization controls (global SaaS)"
        ), sources
    ...
    def _handle_recommendation(self, text: str) -> tuple[str, list[QuerySource]]:
        sources = [
            QuerySource(
                ...
                relevance_score=0.85,
                snippet="Top recommendations",
            )
        ]
        return (
            "Top recommendations to improve compliance:\n"
            ...
            "4. **Update vendor risk assessments** — 3 vendors need re-evaluation (Medium)"
        ), sources
    ...
    async def _enhance_with_ai(
        self, query: str, base_answer: str, intent: QueryIntent
    ) -> str | None:
    ...
    def _generate_follow_ups(self, intent: QueryIntent, text: str) -> list[str]:
        follow_ups: dict[QueryIntent, list[str]] = {
            QueryIntent.REGULATION_LOOKUP: [
                ...
                "Show affected code for this regulation",
            ],
            QueryIntent.VIOLATION_QUERY: [
            ...
            QueryIntent.CODE_SEARCH: [
                "Are there compliance gaps in these files?",
                "Generate compliant code modifications",
            ],
            QueryIntent.AUDIT_QUERY: [
            ...
            QueryIntent.COMPARISON: [
                ...
                "Show overlap between frameworks",
            ],
            QueryIntent.RECOMMENDATION: [
