...
class RiskScorer:
    ...
    async def assess_vendor(
        ...
        assessment.compliance_score = (
            100
            if vendor.compliance_tier == ComplianceTier.FULLY_CERTIFIED
            else (70 if vendor.compliance_tier == ComplianceTier.PARTIALLY_CERTIFIED else 30)
        )
    ...
    async def assess_graph(
        ...
        return {
            ...
            "critical_vendors": [
                ...
                for a in assessments
                if a.overall_risk == RiskLevel.CRITICAL
            ],
            "high_risk_vendors": [
                {"name": a.vendor_name, "score": a.risk_score}
                for a in assessments
                if a.overall_risk == RiskLevel.HIGH
            ],
    ...
    def _generate_required_actions(
        ...
        if "HIPAA" in regulations and any(d in vendor.data_processing for d in ["health", "phi"]):
