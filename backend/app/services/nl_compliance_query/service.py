...
class NLComplianceQueryService:
    ...
    async def _generate_answer(
        self, intent: QueryIntent, entities: dict, raw_query: str
        ...
        answers: dict[QueryIntent, tuple] = {
            QueryIntent.VIOLATION_SEARCH: (
                ...
                [
                    {"severity": "critical", "count": 3, "category": "data_retention"},
                    {"severity": "high", "count": 5, "category": "consent"},
                    {"severity": "medium", "count": 4, "category": "documentation"},
                ],
                [
                    {"type": "regulation", "ref": f"{fw} Art. 5"},
                    {"type": "scan_result", "ref": "scan-2026-02-21"},
                ],
            ),
            QueryIntent.REGULATION_LOOKUP: (
                ...
                [
                    {"article": "Art. 6", "topic": "Lawfulness"},
                    {"article": "Art. 17", "topic": "Right to Erasure"},
                    {"article": "Art. 25", "topic": "Data Protection by Design"},
                ],
            ...
            QueryIntent.POSTURE_STATUS: (
                ...
                [
                    {"metric": "overall_score", "value": 85.2},
                    {"metric": f"{fw.lower()}_score", "value": 88.0},
                    {"metric": "trend", "value": 3.2},
                ],
            ...
            QueryIntent.AUDIT_SEARCH: (
                ...
                [
                    {"event_type": "code_change", "count": 12},
                    {"event_type": "verification", "count": 8},
                    {"event_type": "policy_update", "count": 5},
                ],
            ...
            QueryIntent.COMPARISON: (
                ...
                [
                    {"service": svc, "score": 88},
                    {"service": "payments", "score": 82},
                    {"service": "auth", "score": 91},
                ],
            ...
            QueryIntent.RECOMMENDATION: (
                ...
                [
                    {"recommendation": "Data retention policies", "impact_points": 5},
                    {"recommendation": "Consent management", "impact_points": 3},
                    {"recommendation": "Art. 30 records", "impact_points": 2},
                ],
            ...
            QueryIntent.TREND_ANALYSIS: (
                ...
                [
                    {"date": "start", "score": 81.5},
                    {"date": "end", "score": 85.2},
                    {"metric": "violation_reduction", "value": -33},
                ],
        ...
        return answers.get(
            intent,
            (
                f"I analyzed your query about {fw}. Please refine your question for more specific results.",
                [],
                [],
            ),
        )
    ...
    def _generate_follow_ups(self, intent: QueryIntent, entities: dict) -> list[str]:
        follow_ups: dict[QueryIntent, list[str]] = {
            QueryIntent.VIOLATION_SEARCH: [
                "Show me the critical violations in detail",
            ...
            QueryIntent.POSTURE_STATUS: [
                "Show score breakdown by framework",
                "What's causing the lowest scores?",
    ...
    async def submit_feedback(
        self, query_id: UUID, helpful: bool, comment: str = ""
    ) -> QueryFeedback:
        feedback = QueryFeedback(
            query_id=query_id, helpful=helpful, comment=comment, created_at=datetime.now(UTC)
        )
            ...
            history.append(
                {
                    ...
                    "time_ms": result.execution_time_ms if result else 0,
                }
            )
        return history
    ...
    def get_stats(self) -> QueryStats:
        ...
        return QueryStats(
            ...
            positive_feedback_rate=round(positive / len(self._feedback), 2)
            if self._feedback
            else 0.0,
