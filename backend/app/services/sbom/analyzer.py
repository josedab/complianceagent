...
class SBOMComplianceAnalyzer:
    ...
    async def analyze_compliance(
        ...
        report = SBOMComplianceReport(
            ...
            regulation_compliance=dict.fromkeys(regulations, 100.0),
            ...
            for vuln in component.vulnerabilities:
                ...
                for regulation in regulations:
                    ...
                    issue = ComponentComplianceIssue(
                        ...
                        remediation=f"Upgrade {component.name} to {vuln.fixed_in_version}"
                        if vuln.fixed_in_version
                        else "Review and patch vulnerability",
                        deadline=datetime.now(UTC)
                        + timedelta(days=mapping_info.get("deadline_days", 90)),
                    )
    ...
    async def _analyze_licenses(
        ...
        for component in sbom.components:
            ...
            LICENSE_COMPLIANCE_INFO.get(license_name, {"risk": LicenseRisk.UNKNOWN})
    ...
    async def _analyze_supply_chain(
        ...
        components_without_supplier = [
            c for c in sbom.components if not c.supplier and not c.author
        ]
    ...
    def _calculate_scores(
        ...
        if regulations:
            report.overall_compliance_score = sum(report.regulation_compliance.values()) / len(
                regulations
            )
        else:
        ...
        license_issues = sum(
            1
            for issues in report.issues_by_regulation.values()
            for issue in issues
            if issue.issue_type == "license"
        )
        ...
        supply_chain_issues = sum(
            ...
            for issues in report.issues_by_regulation.values()
            for issue in issues
            if issue.issue_type == "supply_chain"
        )
    ...
    def _generate_recommendations(
        ...
        if report.overall_compliance_score < 70:
            recommendations.append("Consider a comprehensive dependency audit and upgrade strategy")
