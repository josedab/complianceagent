...
class PRBotConfig:
    ...
    enabled_regulations: list[str] = field(
        default_factory=lambda: ["GDPR", "CCPA", "HIPAA", "PCI-DSS", "EU AI Act", "SOX"]
    )
...
class PRBot:
    ...
    async def process_task(
        ...
        try:
            ...
            if self.config.generate_auto_fixes and violations:
                if self.copilot:
                    AutoFixGenerator(copilot_client=self.copilot)
            ...
            if self.config.post_review_comments and task.post_comments:
                ...
                comments = [
                    self.comment_service.create_comment_from_violation(v) for v in violations
                ]
    ...
    async def create_fix_pr(
        ...
        async with client:
            ...
            # Create branch
            branch_name = f"compliance-fix/{pr_number}-{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}"
            ...
            # Create PR
            fix_descriptions = "\n".join(f"- {f.get('description', 'Fix')}" for f in fixes)
...
""",
            ...
            await client.add_labels_to_pr(
                owner, repo, created_pr.number, ["compliance:auto-fix", "bot"]
            )
        ...
        for file_info in changed_files or []:
            ...
            patterns = [
                (
                    ...
                    "Potential PII collection without consent check",
                    "GDPR",
                ),
