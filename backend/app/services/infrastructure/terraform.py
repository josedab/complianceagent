...
class TerraformAnalyzer:
    ...
    def __init__(self, policy_rules: list[PolicyRule] | None = None):
        ...
        self.policy_rules = policy_rules or [
            r for r in DEFAULT_POLICY_RULES if CloudProvider.AWS in r.providers or not r.providers
        ]
    ...
    def _check_rule(
        ...
        if rule.id == "IAM001" and parsed.resource_type in [
            "aws_iam_policy",
            "aws_iam_role_policy",
        ]:
            policy = parsed.properties.get("policy", "")
            if (
                isinstance(policy, str) and '"Action": "*"' in policy
            ) or '"Action": ["*"]' in policy:
                return self._create_violation(
                    rule,
                    parsed,
...
def analyze_terraform_file(
    file_path: str | Path, regulations: list[str] | None = None
) -> tuple[list[InfrastructureResource], list[ComplianceViolation]]:
