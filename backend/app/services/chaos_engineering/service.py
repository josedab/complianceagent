...
_EXPERIMENTS: list[ChaosExperiment] = [
    ChaosExperiment(
        ...
        time_to_remediate_seconds=180.0,
        detection_method="drift_detection_service",
        started_at=datetime.now(UTC) - timedelta(days=7),
        completed_at=datetime.now(UTC) - timedelta(days=7) + timedelta(minutes=5),
    ),
    ChaosExperiment(
        id=uuid4(),
        name="Audit Log Black Hole",
        ...
        time_to_remediate_seconds=None,
        detection_method="none",
        started_at=datetime.now(UTC) - timedelta(days=5),
        completed_at=datetime.now(UTC) - timedelta(days=5) + timedelta(minutes=10),
    ),
    ChaosExperiment(
        id=uuid4(),
        name="PII Exposure Probe",
...
_GAME_DAYS: list[GameDay] = [
    GameDay(
        ...
        team_readiness_score=72.5,
        total_experiments=3,
        experiments_detected=2,
        avg_detection_time_seconds=28.75,
        avg_remediation_time_seconds=120.0,
        scheduled_at=datetime.now(UTC) - timedelta(days=7),
        completed_at=datetime.now(UTC) - timedelta(days=7) + timedelta(hours=2),
    ),
]
...
class ChaosEngineeringService:
    ...
    async def run_experiment(self, experiment_id: UUID) -> ChaosExperiment:
        ...
        if exp.status != ExperimentStatus.SCHEDULED:
            raise ValueError(
                f"Experiment {experiment_id} cannot be run (status: {exp.status.value}); must be SCHEDULED"
            )
    ...
    async def get_stats(self) -> ChaosStats:
        ...
        avg_mttd = sum(
            e.time_to_detect_seconds for e in detected if e.time_to_detect_seconds
        ) / max(len(detected), 1)
        avg_mttr = sum(
            e.time_to_remediate_seconds for e in detected if e.time_to_remediate_seconds
        ) / max(len(detected), 1)
        return ChaosStats(
