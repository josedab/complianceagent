...
class QueryParser:
    ...
    def _extract_time_reference(self, query: str) -> str | None:
        ...
        for pattern, _replacement in time_patterns:
    ...
    def suggest_clarifications(self, parsed: ParsedQuery) -> list[str]:
        ...
        if parsed.intent == QueryIntent.UNKNOWN:
            clarifications.append(
                "Could you rephrase your question? I can help with compliance status, regulation info, code review, or remediation guidance."
            )

        if parsed.intent == QueryIntent.COMPLIANCE_STATUS and not parsed.regulations:
            clarifications.append(
                "Which regulation or framework are you asking about? (e.g., GDPR, HIPAA, SOC2)"
            )
