...
class KnowledgeGraphService:
    ...
    async def _add_evidence_nodes(self, graph: KnowledgeGraph) -> None:
        ...
        logger.info(
            "evidence_nodes_added", controls=sum(len(c) for c in control_frameworks.values())
        )
    ...
    async def _add_risk_nodes(self, graph: KnowledgeGraph) -> None:
        ...
        risk_categories = [
            ...
            (
                ...
                "Risk from third-party vendor non-compliance",
            ),
        ]
    ...
    async def _generate_answer(
        ...
        Node types: {", ".join({n.node_type.value for n in result.nodes[:20]})}
        ...
        try:
            response = await self.copilot.chat.completions.create(
                messages=[
                    {
                        "role": "system",
                        "content": "You are a compliance knowledge expert. Provide a concise answer based on the graph data.",
                    },
    ...
    async def get_node_details(
        ...
        # Get edges
        edges = [e for e in graph.edges if node_id in (e.source_id, e.target_id)]
