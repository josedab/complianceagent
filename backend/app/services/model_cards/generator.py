...
class ModelCardGenerator:
    ...
    def _determine_category(
        ...
        category_keywords = {
            ...
            AISystemCategory.FOUNDATION_MODEL: [
                "llm",
    ...
    def _determine_risk_level(self, category: AISystemCategory) -> AIRiskLevel:
        ...
        if category in [
            AISystemCategory.CHATBOT,
            AISystemCategory.EMOTION_RECOGNITION,
            AISystemCategory.DEEPFAKE,
        ]:
        ...
        if card.regulatory.eu_ai_act_risk_level == AIRiskLevel.HIGH:
            ...
            if not card.regulatory.conformity_assessment_completed:
                ...
                recommendations.append(
                    "Complete conformity assessment before deployment (EU AI Act Article 43)"
                )
    ...
    async def generate_markdown(self, card: ModelCard) -> str:
        ...
        if card.intended_use.human_oversight_required:
            sections.extend(
                [
                    "### Human Oversight",
                    "",
                    card.intended_use.oversight_description
                    or "Human oversight is required for this model.",
                    "",
                ]
            )
        ...
        if (
            card.ethical_considerations.ethical_risks
            or card.ethical_considerations.affected_stakeholders
        ):
            sections.extend(
                [
                    "## Ethical Considerations",
        ...
        sections.extend(
            [
                ...
                "---",
                "",
                f"*Model Card generated by ComplianceAgent on {datetime.now(UTC).strftime('%Y-%m-%d')}*",
            ]
        )
