...
class RelevanceScorer:
    ...
    def score_update(
        ...
        jurisdiction_score = self._score_jurisdiction(update.jurisdiction, profile.jurisdictions)
        ...
        if industry_score > 0.5:
            matched_criteria.append("Industry match")
        ...
        explanation = self._generate_explanation(update, profile, overall, matched_criteria)
    ...
    def predict_impact(
        ...
        effort = effort_map.get((update.severity, update.update_type.value), "low")

        # Calculate days until action required
        days_to_action = None
        if update.effective_date:
            delta = update.effective_date - datetime.now(UTC)
    ...
    def _score_urgency(self, update: RegulatoryUpdate) -> float:
        ...
        if update.effective_date:
            days_until = (update.effective_date - datetime.now(UTC)).days
            if days_until <= 30:
    ...
    def _generate_explanation(
        ...
        if update.effective_date:
            days = (update.effective_date - datetime.now(UTC)).days
            if days > 0:
        ...
        if update.effective_date:
            ...
            if days <= 90:
                recommendations.append(
                    f"Create compliance project plan (deadline: {update.effective_date.date()})"
                )
