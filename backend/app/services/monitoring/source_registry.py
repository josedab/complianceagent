...
def get_source_definitions_by_category(category: str) -> list[dict[str, Any]]:
    ...
    if category not in FRAMEWORK_CATEGORIES:
        raise ValueError(
            f"Unknown category: {category}. Valid categories: {list(FRAMEWORK_CATEGORIES.keys())}"
        )
...
def get_source_definitions_by_framework(framework_key: str) -> list[dict[str, Any]]:
    ...
    if framework_key not in SOURCE_REGISTRY:
        raise ValueError(
            f"Unknown framework: {framework_key}. Valid frameworks: {list(SOURCE_REGISTRY.keys())}"
        )

    return SOURCE_REGISTRY[framework_key]()


async def initialize_all_sources(
    db, frameworks: list[str] | None = None
) -> dict[str, list[RegulatorySource]]:
        ...
        try:
            ...
            for source_def in source_defs:
                ...
                existing = await db.execute(
                    db.query(RegulatorySource).filter(RegulatorySource.name == source_def["name"])
                )
...
def get_framework_statistics() -> dict[str, Any]:
    ...
    for source in all_sources:
        jurisdiction = (
            ...
            if hasattr(source["jurisdiction"], "value")
            else str(source["jurisdiction"])
        )
