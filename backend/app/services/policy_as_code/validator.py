...
class PolicyValidator:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if " = " in stripped and not stripped.startswith("#"):
                if "default " not in stripped and ":= " not in stripped:
                    warnings.append(
                        f"Line {i}: Consider using := instead of = for rule definitions"
                    )
    ...
    async def _validate_semantics(self, package: PolicyPackage) -> dict[str, Any]:
        ...
        unknown_regs = [
            r.regulation for r in package.rules if r.regulation not in known_regulations
        ]
        if unknown_regs:
    ...
    async def _run_single_test(
        ...
        try:
            ...
            passed = actual_result == test_case.expected_result and set(actual_violations) == set(
    ...
    async def _run_test_with_opa(
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                return {
                    "allow": result.get("result", [{}])[0]
                    .get("expressions", [{}])[0]
                    .get("value", False),
                    "violations": result.get("result", [{}])[1]
                    .get("expressions", [{}])[0]
                    .get("value", []),
                }
