...
class PolicyGenerator:
    ...
    async def create_custom_package(
        ...
        regulations = list({r.regulation for r in policy_rules})
    ...
    async def _condition_to_rego(self, rule: PolicyRule) -> str:
        ...
        if category == PolicyCategory.DATA_RETENTION:
            return self._generate_retention_check()
        if category == PolicyCategory.CONSENT:
            return self._generate_consent_check()
        if category == PolicyCategory.BREACH_NOTIFICATION:
            return self._generate_breach_check()
        if category == PolicyCategory.VULNERABILITY:
            return self._generate_vulnerability_check()
        if category == PolicyCategory.AI_GOVERNANCE:
    ...
    async def generate_policy_bundle(self, framework: str, output_format: str = "yaml") -> dict:
        ...
        if output_format == "yaml":
            bundle["content"] = (
                f"# {framework} Compliance Policy Bundle\nversion: 1.0.0\nframework: {framework}\npolicies: []\n"
            )
        elif output_format == "rego":
            bundle["content"] = (
                f"package compliance.{framework}\n\ndefault allow = false\n\n# {framework} compliance rules\n"
            )
        elif output_format == "python":
            bundle["content"] = (
                f'"""Compliance policy bundle for {framework}."""\n\nFRAMEWORK = "{framework}"\nVERSION = "1.0.0"\n\ndef check_compliance(resource: dict) -> bool:\n    return True\n'
            )
...
""",
        ...
        return {
            ...
            "files": {
                ...
                for r, t in zip(package.rules, constraint_templates, strict=False)
            },
    ...
    async def _export_kyverno(self, package: PolicyPackage) -> dict[str, Any]:
        ...
        return {
            ...
            "files": {
                ...
                for r, p in zip(package.rules, policies, strict=False)
            },
