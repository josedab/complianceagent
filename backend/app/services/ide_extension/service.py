...
_COMPLIANCE_RULES: list[dict] = [
    {
        ...
        "fix": "# TODO: Add GDPR consent check\nassert has_consent(user_id)",
    },
    {
        "rule": "gdpr-retention",
        "framework": "GDPR",
        "article": "Art. 5(1)(e)",
        "pattern": "store_forever|no_expiry|permanent",
        "severity": "error",
        "message": "Data must have defined retention periods",
        "fix": "expiry = datetime.now() + timedelta(days=RETENTION_DAYS)",
    },
    {
        "rule": "hipaa-phi",
        "framework": "HIPAA",
        ...
        "fix": "encrypted_data = encrypt_phi(data)",
    },
    {
        "rule": "pci-card-data",
        "framework": "PCI-DSS",
    ...
    },
    {
        ...
        "framework": "SOC 2",
        "article": "CC7.1",
        "pattern": "admin_action|privilege_escalation|security_event",
        "severity": "warning",
        "message": "Administrative actions require audit logging",
        "fix": "audit_log.record(action=action, actor=current_user)",
    },
]

_REGULATION_TOOLTIPS: dict[str, RegulationTooltip] = {
    "GDPR Art. 5": RegulationTooltip(
        regulation="GDPR",
        article="Art. 5",
        title="Principles relating to processing of personal data",
        summary="Personal data must be processed lawfully, fairly, transparently; collected for specified purposes; adequate, relevant, limited; accurate; stored with time limits; processed securely.",
        url="https://gdpr-info.eu/art-5-gdpr/",
        obligation_level="must",
    ),
    "GDPR Art. 17": RegulationTooltip(
        regulation="GDPR",
        article="Art. 17",
        title="Right to erasure ('right to be forgotten')",
        summary="Data subject has right to erasure of personal data without undue delay when data is no longer necessary, consent withdrawn, or unlawfully processed.",
        url="https://gdpr-info.eu/art-17-gdpr/",
        obligation_level="must",
    ),
    "HIPAA §164.312": RegulationTooltip(
        regulation="HIPAA",
        article="§164.312",
        title="Technical Safeguards",
        summary="Implement access controls, audit controls, integrity controls, person/entity authentication, and transmission security for ePHI.",
        url="https://www.hhs.gov/hipaa/",
        obligation_level="must",
    ),
    "PCI-DSS Req 3": RegulationTooltip(
        regulation="PCI-DSS",
        article="Req 3",
        title="Protect Stored Account Data",
        summary="Keep cardholder data storage to minimum; do not store sensitive authentication data after authorization; mask PAN when displayed; render PAN unreadable anywhere it is stored.",
...
class IDEExtensionService:
    ...
    async def analyze_file(
        self, file_path: str, content: str, frameworks: list[str] | None = None
    ...
    async def get_quick_fix(
        self, diagnostic_id: str, file_path: str, line: int, rule_id: str
    ) -> IDEQuickFix | None:
