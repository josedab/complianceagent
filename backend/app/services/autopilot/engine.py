...
class AutopilotEngine:
    ...
    async def analyze_violations(
        ...
        plan = RemediationPlan(
            ...
            name=f"Remediation Plan - {datetime.now(UTC).strftime('%Y-%m-%d %H:%M')}",
    ...
    def _determine_action_type(self, violation: ComplianceViolation) -> RemediationType:
        ...
        if any(
            kw in rule_lower or kw in desc_lower for kw in ["access", "permission", "role", "auth"]
        ):
        ...
        if any(
            kw in rule_lower or kw in desc_lower for kw in ["vulnerability", "cve", "dependency"]
        ):
    ...
    async def _generate_code_fix(
        ...
        for pattern_config in CODE_FIX_PATTERNS.values():
    ...
    def _suggest_fix(self, violation: ComplianceViolation) -> str | None:
        ...
        if "hardcoded" in desc_lower and "secret" in desc_lower:
            return (
                "# TODO: Move secret to environment variable or secrets manager\n"
    ...
    async def execute_plan(
        ...
        plan.pending_count = len([a for a in plan.actions if a.status == RemediationStatus.PENDING])
