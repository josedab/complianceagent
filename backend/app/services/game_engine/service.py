...
_SCENARIOS: list[GameScenario] = [
    GameScenario(
        ...
        frameworks=["GDPR"],
        learning_objectives=[
            "Understand 72-hour notification deadline",
        ...
        decisions=[
            GameDecision(
                id="d1",
                prompt="You discover a database backup was publicly accessible for 6 hours. What is your first action?",
                options=[
                    "Immediately notify the DPA",
                    "Assess the scope and impact first",
                    "Delete the backup and hope no one noticed",
                    "Email the engineering team",
                ],
                correct_option=1,
                points=20,
                explanation="GDPR Art. 33 requires assessment before notification. You need to understand scope first.",
                regulation_reference="GDPR Article 33",
            ),
            GameDecision(
                id="d2",
                prompt="Your assessment reveals 12,000 EU citizens' data was exposed. Within how many hours must you notify the supervisory authority?",
                options=["24 hours", "48 hours", "72 hours", "7 days"],
                correct_option=2,
                points=20,
                explanation="GDPR Article 33(1) requires notification within 72 hours of becoming aware.",
                regulation_reference="GDPR Article 33(1)",
            ),
            GameDecision(
                id="d3",
                prompt="Which of the following must be included in your breach notification?",
                options=[
                    "Only the date of the breach",
                    "Nature of breach, categories of data, likely consequences, measures taken",
                    "Just the number of affected individuals",
                    "A formal apology letter",
                ],
                correct_option=1,
                points=25,
                explanation="Art. 33(3) requires: nature, categories, consequences, and measures taken or proposed.",
    ...
    GameScenario(
        ...
        decisions=[
            GameDecision(
                id="d1",
                prompt="The auditor requests your Risk Analysis documentation. What should this include?",
                options=[
                    "Just a list of servers",
                    "Threats, vulnerabilities, likelihood, impact for all ePHI",
                    "Your latest penetration test report",
                    "Employee training records",
                ],
                correct_option=1,
                points=25,
                explanation="45 CFR § 164.308(a)(1) requires comprehensive risk analysis covering all ePHI.",
                regulation_reference="45 CFR § 164.308(a)(1)",
            ),
            GameDecision(
                id="d2",
                prompt="The auditor finds that your application logs contain patient names in plaintext. What is the correct response?",
                options=[
                    "It's fine since logs are internal",
                    "Acknowledge the finding and implement log scrubbing immediately",
                    "Argue that log data isn't ePHI",
                    "Ask for a 30-day remediation window",
                ],
                correct_option=1,
                points=30,
                explanation="Patient names in logs constitute ePHI exposure. Immediate remediation is expected.",
    ...
    GameScenario(
        ...
        decisions=[
            GameDecision(
                id="d1",
                prompt="Ransomware has encrypted your production database. What is your immediate priority?",
                options=[
                ...
                correct_option=1,
                points=30,
                explanation="Containment is the first priority to prevent lateral movement.",
                regulation_reference="NIST SP 800-61r2",
            ),
            GameDecision(
                id="d2",
                prompt="After containment, you discover PCI cardholder data may have been exfiltrated. What must you do?",
                options=[
                    "Wait for forensic analysis to complete",
                    "Notify your acquiring bank and payment brands within 24 hours",
                    "Only notify if data was confirmed stolen",
                    "Reset all passwords and move on",
                ],
                correct_option=1,
                points=35,
                explanation="PCI-DSS requires immediate notification to acquirer and payment brands.",
    ...
    GameScenario(
        ...
        learning_objectives=[
            "Identify data exposure vectors",
        ...
        decisions=[
            GameDecision(
                id="d1",
                prompt="The error response includes user email, IP address, and stack trace. Which data should be removed first?",
                options=[
                    "Stack trace only",
                    "All PII (email, IP) and stack trace",
                    "Just the email",
                    "None - it helps debugging",
                ],
                correct_option=1,
                points=20,
                explanation="All PII and internal details must be removed from error responses per data minimization principles.",
    ...
    GameScenario(
        ...
        frameworks=["GDPR", "SOC 2"],
        learning_objectives=[
            "Understand processor obligations",
            "Assess shared responsibility",
            "Execute vendor incident workflow",
        ],
        decisions=[
            GameDecision(
                id="d1",
                prompt="Your cloud vendor notifies you of a breach. As the data controller, what is your obligation?",
                options=[
                    "Nothing - it's the vendor's responsibility",
                    "Conduct your own assessment and notify your supervisory authority if required",
                    "Sue the vendor immediately",
                    "Switch vendors and don't notify anyone",
                ],
                correct_option=1,
                points=25,
                explanation="As controller, you remain responsible for PII even when processed by a vendor.",
...
_ACHIEVEMENTS: list[Achievement] = [
    Achievement(
        id="gdpr-guardian",
        name="GDPR Guardian",
        description="Complete all GDPR scenarios",
    ...
    Achievement(
        ...
        description="Score 90%+ on a breach scenario",
    ...
    Achievement(
        id="perfect-score",
        name="Perfect Score",
        description="Achieve 100% on any scenario",
    ...
    Achievement(
        id="multi-framework",
        name="Multi-Framework Expert",
        description="Complete scenarios across 3+ frameworks",
        tier=AchievementTier.GOLD,
        icon="🌐",
        xp_reward=100,
    ),
...
class GameEngineService:
    ...
    async def submit_decision(
        ...
        if selected_option < 0 or selected_option >= len(decision.options):
            raise ValueError(
                f"selected_option {selected_option} out of bounds (0 to {len(decision.options) - 1})"
            )
    ...
    async def get_player_progress(self, player_id: UUID) -> list[PlayerProgress]:
        return [
            PlayerProgress(
                player_id=player_id,
                scenario_id="gdpr-breach-response",
                score=85,
                decisions_made=3,
                correct_decisions=2,
