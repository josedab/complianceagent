...
_REGULATION_CONTEXTS: dict[str, list[RegulationContext]] = {
    "python": [
        RegulationContext(
            ...
            applicable_patterns=["encryption", "hashing", "pii_handling"],
        ),
        RegulationContext(
            regulation="HIPAA",
            article="§164.312(a)",
            ...
            applicable_patterns=["authentication", "authorization", "rbac"],
        ),
    ],
    "typescript": [
        RegulationContext(
            ...
            applicable_patterns=["consent_form", "cookie_banner", "opt_in"],
        ),
        RegulationContext(
            regulation="PCI-DSS",
            article="Req. 3",
            ...
            applicable_patterns=["card_number", "payment", "tokenization"],
        ),
    ],
}
...
_COMPLIANCE_RULES: list[dict] = [
    {
        ...
        "pattern": "email|phone|ssn|name.*=",
        "regulation": "GDPR",
        "article": "Art. 5(1)(c)",
        "message": "PII detected without masking or encryption",
        "severity": "error",
        "fix": "Apply data masking: mask_pii(value)",
    },
    {
        "rule_id": "GDPR-LOG-002",
        "pattern": "print\\(.*user|log.*password",
        "regulation": "GDPR",
        "article": "Art. 5(1)(f)",
        "message": "Potential PII in log output",
        "severity": "warning",
        "fix": "Use structured logging with PII redaction",
    },
    {
        "rule_id": "HIPAA-ENC-001",
        "pattern": "password.*=.*plain|store.*unencrypted",
        "regulation": "HIPAA",
        "article": "§164.312(a)(2)(iv)",
        "message": "Unencrypted ePHI storage detected",
        "severity": "error",
        "fix": "Use AES-256 encryption: encrypt(data, key)",
    },
    {
        "rule_id": "PCI-CARD-001",
        "pattern": "card.*number|credit_card|pan\\b",
        "regulation": "PCI-DSS",
        "article": "Req. 3.4",
        "message": "Cardholder data without tokenization",
        "severity": "error",
        "fix": "Replace with tokenized reference: tokenize(card_number)",
    },
    {
        "rule_id": "SOC2-AUTH-001",
        "pattern": "admin.*password|hardcoded.*secret",
        "regulation": "SOC 2",
        "article": "CC6.1",
    ...
    # --- GDPR Data Retention & Erasure ---
    {
        ...
        "pattern": "retention.*forever|never.*delete|keep.*indefinite",
        "regulation": "GDPR",
        "article": "Art. 5(1)(e)",
        "message": "Data retained beyond necessity — storage limitation principle",
        "severity": "error",
        "fix": "Implement retention policies with automatic expiry: set_retention(data, max_days=365)",
    },
    {
        "rule_id": "GDPR-RET-002",
        "pattern": "ttl.*=.*0|expir.*none|no.*expiry",
        "regulation": "GDPR",
        "article": "Art. 5(1)(e)",
        "message": "Missing data expiration — storage limitation violation",
        "severity": "warning",
        "fix": "Set appropriate TTL: ttl=timedelta(days=retention_period)",
    },
    {
        "rule_id": "GDPR-DEL-001",
        "pattern": "soft.*delete|mark.*deleted|is_deleted",
        "regulation": "GDPR",
        "article": "Art. 17",
        "message": "Soft delete may not satisfy right to erasure requirements",
        "severity": "warning",
        "fix": "Implement hard delete with cascade: hard_delete_user_data(user_id)",
    },
    {
        "rule_id": "GDPR-DEL-002",
        "pattern": "backup.*user|archive.*personal",
        "regulation": "GDPR",
        "article": "Art. 17",
        "message": "Archived personal data must also support erasure",
        "severity": "info",
        "fix": "Ensure backups have erasure capability: register_erasure_handler(backup_store)",
    },
    # --- HIPAA Audit & Minimum Necessary ---
    {
        "rule_id": "HIPAA-AUD-001",
        "pattern": "patient.*record|medical.*data|health.*info",
        "regulation": "HIPAA",
        "article": "§164.312(b)",
        "message": "ePHI access without audit logging",
        "severity": "error",
        "fix": "Add audit trail: audit_log.record_access(user, resource, action)",
    },
    {
        "rule_id": "HIPAA-AUD-002",
        "pattern": "select.*\\*.*from.*patient|fetch_all.*medical",
        "regulation": "HIPAA",
        "article": "§164.312(b)",
        "message": "Broad ePHI query without audit trail",
        "severity": "warning",
        "fix": "Log query scope: audit_log.record_query(user, table, filter_criteria)",
    },
    {
        "rule_id": "HIPAA-MIN-001",
        "pattern": "select.*\\*.*from.*user|fetch_all_fields|get_complete_record",
        "regulation": "HIPAA",
        "article": "§164.502(b)",
        "message": "Fetching all fields violates minimum necessary principle",
        "severity": "warning",
        "fix": "Select only required fields: select_fields(table, ['name', 'appointment_date'])",
    },
    {
        "rule_id": "HIPAA-MIN-002",
        "pattern": "return.*all.*data|dump.*patient|export.*records",
        "regulation": "HIPAA",
        "article": "§164.502(b)",
        "message": "Exporting full records without minimum necessary filtering",
        "severity": "error",
        "fix": "Apply field-level filtering: export_filtered(records, allowed_fields)",
    },
    # --- PCI-DSS Input Validation & Session ---
    {
        "rule_id": "PCI-INP-001",
        "pattern": "eval\\(|exec\\(|os\\.system\\(|subprocess.*shell.*=.*True",
        "regulation": "PCI-DSS",
        "article": "Req. 6.5",
        "message": "Unsafe code execution — injection vulnerability",
        "severity": "error",
        "fix": "Use parameterized calls: subprocess.run(args, shell=False)",
    },
    {
        "rule_id": "PCI-INP-002",
        "pattern": "request\\.get\\(|params\\[|query_string",
        "regulation": "PCI-DSS",
        "article": "Req. 6.5",
        "message": "User input used without validation",
        "severity": "warning",
        "fix": "Validate and sanitize: validated = validate_input(raw_input, schema)",
    },
    {
        "rule_id": "PCI-SES-001",
        "pattern": "session.*expire.*=.*0|session.*timeout.*none|no.*session.*limit",
        "regulation": "PCI-DSS",
        "article": "Req. 6.5.10",
        "message": "Missing session timeout configuration",
        "severity": "error",
        "fix": "Set session timeout: session.configure(timeout_minutes=15)",
    },
    {
        "rule_id": "PCI-SES-002",
        "pattern": "session_id.*url|token.*query.*param|jwt.*url",
        "regulation": "PCI-DSS",
        "article": "Req. 6.5.10",
        "message": "Session token exposed in URL",
        "severity": "error",
        "fix": "Use secure cookies or headers: response.set_cookie(token, httponly=True, secure=True)",
    },
    # --- SOC 2 Change Management & Monitoring ---
    {
        "rule_id": "SOC2-CHG-001",
        "pattern": "deploy.*prod.*direct|push.*master|force.*push",
        "regulation": "SOC 2",
        "article": "CC8.1",
        "message": "Direct production deployment without change management",
        "severity": "error",
        "fix": "Use CI/CD pipeline with approvals: deploy_via_pipeline(artifact, approvers)",
    },
    {
        "rule_id": "SOC2-CHG-002",
        "pattern": "migrate.*run|alter.*table|schema.*change",
        "regulation": "SOC 2",
        "article": "CC8.1",
        "message": "Schema change without change management tracking",
        "severity": "warning",
        "fix": "Track migrations: register_change(migration_id, reviewer, rollback_plan)",
    },
    {
        "rule_id": "SOC2-MON-001",
        "pattern": "except.*pass|catch.*ignore|silent.*fail",
        "regulation": "SOC 2",
        "article": "CC7.2",
        "message": "Silent error suppression hinders monitoring",
        "severity": "warning",
        "fix": "Log exceptions: logger.error('operation_failed', error=str(e), context=ctx)",
    },
    {
        "rule_id": "SOC2-MON-002",
        "pattern": "disable.*alert|skip.*monitor|no.*metric",
        "regulation": "SOC 2",
        "article": "CC7.2",
        "message": "Monitoring or alerting disabled",
        "severity": "error",
        "fix": "Maintain monitoring: metrics.track(operation, status, duration)",
    },
    # --- CCPA Do-Not-Sell ---
    {
        "rule_id": "CCPA-DNS-001",
        "pattern": "share.*third.?party|send.*partner.*data|export.*vendor",
        "regulation": "CCPA",
        "article": "§1798.120",
        "message": "Data sharing without do-not-sell check",
        "severity": "error",
        "fix": "Check opt-out status: if not user.do_not_sell: share_data(partner, data)",
    },
    {
        "rule_id": "CCPA-DNS-002",
        "pattern": "sell.*data|monetize.*user|data.*broker",
        "regulation": "CCPA",
        "article": "§1798.120",
        "message": "Data sale without consumer opt-out mechanism",
        "severity": "error",
        "fix": "Implement opt-out: require_consent(user_id, purpose='data_sale')",
    },
    # --- EU AI Act Transparency ---
    {
        "rule_id": "EUAI-TRA-001",
        "pattern": "predict\\(|model\\.run|inference|classify\\(",
        "regulation": "EU AI Act",
        "article": "Art. 13",
        "message": "AI inference without transparency logging",
        "severity": "warning",
        "fix": "Log AI decisions: log_ai_decision(model_id, input_hash, output, confidence)",
    },
    {
        "rule_id": "EUAI-TRA-002",
        "pattern": "auto.*decision|automated.*reject|bot.*approve",
        "regulation": "EU AI Act",
        "article": "Art. 13",
        "message": "Automated decision-making without explainability",
        "severity": "error",
        "fix": "Provide explanations: explain_decision(model, input, output, factors)",
    },
]
        ...
        for i, line in enumerate(lines, 1):
            for rule in _COMPLIANCE_RULES:
                try:
                    ...
                    logger.warning(
                        "pair.bad_regex", rule_id=rule["rule_id"], pattern=rule["pattern"]
                    )
                    continue
                if match:
                    suggestions.append(
    ...
    async def get_refactoring_suggestions(
        ...
        if len(crypto_files) > 1:
            suggestions.append(
                RefactoringSuggestion(
                    id=uuid4(),
                    title="Unify cryptographic operations",
