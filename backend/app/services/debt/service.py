...
_DEBT_ITEMS: list[ComplianceDebtItem] = [
    ...
    ComplianceDebtItem(
        id=uuid4(),
        title="Incomplete audit logging on admin endpoints",
        description="15 admin endpoints lack structured audit logging for SOC 2 CC7.2.",
        priority=DebtPriority.HIGH,
        framework="SOC 2",
        control="CC7.2",
        risk_score=7.5,
        cost_of_delay_per_day=85.0,
        accrued_interest=1700.0,
        remediation_cost=1200.0,
        days_outstanding=20,
    ...
    ComplianceDebtItem(
        ...
        control="Req. 6.2",
        risk_score=6.8,
        cost_of_delay_per_day=65.0,
        accrued_interest=975.0,
    ...
    ComplianceDebtItem(
        id=uuid4(),
        title="Missing consent banner for analytics cookies",
        description="Analytics tracking runs before user consent on EU visitors.",
        priority=DebtPriority.MEDIUM,
        framework="GDPR",
        control="Art. 7",
        risk_score=5.5,
        cost_of_delay_per_day=40.0,
        accrued_interest=400.0,
    ...
    ComplianceDebtItem(
        ...
        control="Art. 5(1)(e)",
        risk_score=6.0,
        cost_of_delay_per_day=55.0,
        accrued_interest=1650.0,
        remediation_cost=1500.0,
        days_outstanding=30,
        assigned_team="Backend",
    ),
    ComplianceDebtItem(
        id=uuid4(),
        title="Password policy below NIST guidelines",
        description="Current: 8 chars min, no complexity. NIST SP 800-63B recommends 8+ with breach checks.",
        priority=DebtPriority.LOW,
        framework="SOC 2",
        control="CC6.1",
        risk_score=4.0,
        cost_of_delay_per_day=25.0,
        accrued_interest=250.0,
        remediation_cost=600.0,
        days_outstanding=10,
        assigned_team="Security",
    ),
]
...
class DebtSecuritizationService:
    ...
    async def list_debt_items(
        self,
        priority: DebtPriority | None = None,
    ...
    async def get_portfolio(self) -> DebtPortfolio:
        ...
        for d in _DEBT_ITEMS:
            by_framework[d.framework] = (
                by_framework.get(d.framework, 0) + d.remediation_cost + d.accrued_interest
            )

        return DebtPortfolio(
            total_debt_value=total,
            total_items=len(_DEBT_ITEMS),
            ...
            debt_by_framework=by_framework,
            remediation_velocity=2.5,
            projected_payoff_days=int(total / (daily_rate * 0.3)),
        )

    async def get_bonds(self) -> list[ComplianceBond]:
        bonds = []
        frameworks = {d.framework for d in _DEBT_ITEMS}
        for fw in frameworks:
            ...
            bonds.append(
                ComplianceBond(
                    id=uuid4(),
                    name=f"{fw} Compliance Bond",
                    face_value=face,
                    current_value=face * 1.05,
                    interest_rate=0.05,
                    framework=fw,
                    debt_items_count=len(items),
                    yield_spread=0.02,
                    maturity_date=datetime.now(UTC) + timedelta(days=90),
                )
            )
        return bonds
