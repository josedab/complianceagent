...
class RegulatorySandboxIntegration:
    ...
    async def submit_application(
        ...
        mandatory_met = all(r.status == "met" for r in app.requirements if r.mandatory)
    ...
    async def check_pre_submission(
        ...
        results["met_requirements"] = (
            len(app.requirements)
            - len(results["missing_mandatory"])
            - len(results["missing_optional"])
        )
    ...
    async def create_test(
        ...
        logger.info(
            "Created sandbox test", test_id=str(test.id), application_id=str(application_id)
        )
    ...
    async def get_test_results(
        ...
        return [t for t in self._tests.values() if t.application_id == application_id]
        ...
        return {
            ...
            "requirements_summary": {
                ...
                "completion_percentage": (
                    app.requirements_met_count / app.requirements_total_count * 100
                )
                if app.requirements_total_count
                else 0,
            },
            "testing_summary": {
