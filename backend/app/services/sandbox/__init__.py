...
class ComplianceSandbox:
    ...
    async def run_simulation(
        ...
        compliance_before = current_state.get(
            "compliance_scores",
            {
                "GDPR": 85.0,
                "CCPA": 90.0,
        ...
        if scenario.simulation_type == SimulationType.CODE_CHANGE:
            ...
            result = await self._simulate_architecture_change(
                scenario, current_state, compliance_before
            )
        elif scenario.simulation_type == SimulationType.DATA_FLOW_CHANGE:
            result = await self._simulate_data_flow_change(
                scenario, current_state, compliance_before
            )
        elif scenario.simulation_type == SimulationType.REGULATION_CHANGE:
            result = await self._simulate_regulation_change(
                scenario, current_state, compliance_before
            )
    ...
    async def _simulate_code_change(
        ...
        before_avg = (
            sum(compliance_before.values()) / len(compliance_before) if compliance_before else 0
        )
        after_avg = (
            sum(compliance_after.values()) / len(compliance_after) if compliance_after else 0
        )
        ...
        if new_issues:
            recommendations.append(
                f"Address {len(new_issues)} new compliance issues before merging"
            )
    ...
    async def _simulate_architecture_change(
        ...
        for component in new_components:
            ...
            if "third-party" in component_type.lower() or "external" in component_type.lower():
                ...
                recommendations.append(
                    f"Complete vendor risk assessment for {component.get('name')}"
                )
        ...
        for flow in data_flows:
            if flow.get("crosses_border"):
                ...
                recommendations.append(
                    "Implement Standard Contractual Clauses for cross-border transfer"
                )
    ...
    async def delete_scenario(self, scenario_id: UUID) -> bool:
        ...
        result_ids_to_remove = [
            rid for rid, r in self._results.items() if r.scenario_id == scenario_id
        ]
    ...
    async def compare_scenarios(
        ...
        tasks = [self.run_simulation(sid, current_state) for sid in scenario_ids]
