...
class MigrationPlanner:
    ...
    async def generate_plan(
        ...
        plan = MigrationPlan(
            ...
            name=f"Compliance Migration Plan - {datetime.now(UTC).strftime('%Y-%m-%d')}",
    ...
    async def _generate_tasks_from_issues(
        ...
        sorted_issues = sorted(
            simulation_result.new_issues, key=lambda i: severity_order.get(i.severity, 4)
        )
    ...
    async def _generate_pr_suggestions(
        ...
        for reg, tasks in by_regulation.items():
            pr = {
                ...
                "files_affected": list({f for t in tasks for f in t.related_files}),
    ...
    def _assess_risks(
        ...
        critical_count = sum(1 for t in plan.tasks if t.priority == TaskPriority.CRITICAL)
        if critical_count > 5:
        ...
        if plan.total_estimated_hours > 200:
            risk_factors.append(
                f"Large effort estimate ({plan.total_estimated_hours}h) may require phased approach"
            )

        # Multiple regulations
        if len(plan.regulations_addressed) > 3:
            risk_factors.append(
                f"Multiple regulations ({len(plan.regulations_addressed)}) increase complexity"
            )
        ...
        if score_gap > 0.3:
            risk_factors.append(
                f"Large compliance gap ({score_gap:.0%}) requires significant changes"
            )
