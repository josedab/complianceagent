...
class SnapshotManager:
    ...
    async def _analyze_compliance(
        ...
        for reg in regulations:
            ...
            score = _deterministic_float("seed_2", 0.6, 1.0)
            issues_count = _deterministic_int("seed_1", 0, 10)
        ...
        if snapshot_id in self._snapshots:
            ...
            if snapshot.organization_id and snapshot.organization_id in self._by_org:
                self._by_org[snapshot.organization_id] = [
                    sid for sid in self._by_org[snapshot.organization_id] if sid != snapshot_id
                ]
            return True
        return False
