...
_MARKETS: list[PredictionMarket] = [
    PredictionMarket(
        id=uuid4(),
        title="EU AI Act Enforcement by Feb 2026",
        ...
        closes_at=datetime.now(UTC) + timedelta(days=180),
    ),
    PredictionMarket(
        id=uuid4(),
        title="US Federal Privacy Law by 2025",
        ...
        closes_at=datetime.now(UTC) + timedelta(days=120),
    ),
    PredictionMarket(
        id=uuid4(),
        title="GDPR Fine >€100M in 2025",
...
_LEADERBOARD: list[PredictionRecord] = [
    PredictionRecord(
        ...
        total_pnl=2340.0,
        rank=1,
        is_superforecaster=True,
    ),
    PredictionRecord(
        participant_id=uuid4(),
        display_name="PolicyPredictor",
...
class PredictionMarketService:
    ...
    async def place_prediction(
        ...
        cost = shares * (
            market.current_probability if position == "yes" else 1 - market.current_probability
        )
        pos = MarketPosition(
            id=uuid4(),
            market_id=market_id,
