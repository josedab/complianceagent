...
class VendorAssessmentService:
    ...
    async def list_vendors(
        ...
        vendors = [v for v in self._vendors.values() if v.organization_id == organization_id]
    ...
    async def assess_vendor(
        ...
        for framework in frameworks:
            assessment.framework_assessments[framework] = self._assess_framework_compliance(
                vendor, framework
            )
        ...
        if assessment.overall_score >= 80 and not assessment.compliance_gaps:
            ...
            assessment.valid_until = datetime.now(UTC) + timedelta(days=365)
        elif assessment.overall_score >= 60:
            assessment.status = VendorStatus.CONDITIONALLY_APPROVED
            assessment.conditions = [gap["description"] for gap in assessment.compliance_gaps[:3]]
    ...
    async def list_assessments(
        ...
        """List assessments."""
        assessments = [
            a for a in self._assessments.values() if a.organization_id == organization_id
        ]
    ...
    def _assess_dependency_risk(
        ...
        if dep.license_risk == "critical":
            score += 30
            risk.license_issues.append("AGPL/SSPL license requires source disclosure")
