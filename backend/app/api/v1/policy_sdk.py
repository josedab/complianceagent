...
from app.services.policy_sdk import PolicyCategory, PolicyLanguage, PolicySDKService, PolicySeverity
...
async def list_policies(
    db: DB, copilot: CopilotDep, category: str | None = None, framework: str | None = None
) -> list[PolicySchema]:
    service = PolicySDKService(db=db, copilot_client=copilot)
    cat = (
        PolicyCategory(category)
        if category and category in [c.value for c in PolicyCategory]
        else None
    )
...
async def create_policy(request: CreatePolicyRequest, db: DB, copilot: CopilotDep) -> PolicySchema:
    service = PolicySDKService(db=db, copilot_client=copilot)
    lang = (
        PolicyLanguage(request.language)
        if request.language in [l.value for l in PolicyLanguage]
        else PolicyLanguage.YAML
    )
    cat = (
        PolicyCategory(request.category)
        if request.category in [c.value for c in PolicyCategory]
        else PolicyCategory.CUSTOM
    )
    sev = (
        PolicySeverity(request.severity)
        if request.severity in [s.value for s in PolicySeverity]
        else PolicySeverity.MEDIUM
    )
    ...
    return PolicySchema(
        ...
        language=policy.language.value,
        category=policy.category.value,
        severity=policy.severity.value,
        frameworks=policy.frameworks,
        is_community=policy.is_community,
        author=policy.author,
    )


@router.post(
    "/policies/{policy_id}/validate", response_model=ValidationSchema, summary="Validate policy"
)
...
async def publish_policy(
    policy_id: str, request: PublishRequest, db: DB, copilot: CopilotDep
) -> dict:
    service = PolicySDKService(db=db, copilot_client=copilot)
