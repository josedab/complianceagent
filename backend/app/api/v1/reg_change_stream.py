...
@router.post(
    "/changes",
    response_model=RegulatoryChangeSchema,
    status_code=status.HTTP_201_CREATED,
    summary="Publish regulatory change",
)
async def publish_change(request: PublishChangeRequest, db: DB) -> RegulatoryChangeSchema:
    ...
    return RegulatoryChangeSchema(
        ...
        affected_frameworks=result.affected_frameworks,
...
@router.get(
    "/changes", response_model=list[RegulatoryChangeSchema], summary="List regulatory changes"
)
async def list_changes(
    ...
    limit: int = 50,
) -> list[RegulatoryChangeSchema]:
    service = RegChangeStreamService(db=db)
    s = ChangeSeverity(severity) if severity else None
    changes = service.get_changes(
        severity=s, jurisdiction=jurisdiction, regulation=regulation, limit=limit
    )
    return [
...
async def get_stats(db: DB) -> StreamStatsSchema:
    ...
    return StreamStatsSchema(
        ...
        changes_by_severity=stats.changes_by_severity,
