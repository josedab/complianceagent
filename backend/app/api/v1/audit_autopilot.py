...
def _parse_framework(fw: str) -> AuditFramework:
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported framework: {fw}. Use: {[f.value for f in AuditFramework]}",
        ) from exc


@router.post(
    "/gap-analysis/{framework}", response_model=GapAnalysisSchema, summary="Run gap analysis"
)
async def run_gap_analysis(framework: str, db: DB, copilot: CopilotDep) -> GapAnalysisSchema:
    ...
    return GapAnalysisSchema(
        ...
        controls_met=analysis.controls_met,
        controls_partial=analysis.controls_partial,
...
@router.post(
    "/evidence-package/{framework}",
    response_model=EvidencePackageSchema,
...
async def generate_evidence_package(
    ...
    return EvidencePackageSchema(
        ...
        controls_covered=package.controls_covered,
...
@router.post(
    "/readiness-report/{framework}",
    response_model=ReadinessReportSchema,
    summary="Generate readiness report",
)
