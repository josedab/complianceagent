...
async def get_timeline(
    ...
    view = await service.get_timeline(
        framework=framework, jurisdiction=jurisdiction, days_ahead=days_ahead
    )
    return TimelineSchema(
        events=[
            EventSchema(
                ...
                affected_repos=e.affected_repos,
        ...
        upcoming_deadlines=view.upcoming_deadlines,
        overdue_count=view.overdue_count,
        total_effort_hours=view.total_effort_hours,
    )
...
async def add_event(request: AddEventRequest, db: DB, copilot: CopilotDep) -> EventSchema:
    ...
    return EventSchema(
        ...
        affected_repos=event.affected_repos,
...
@router.post(
    "/events/{event_id}/tasks", response_model=list[TaskSchema], summary="Generate tasks from event"
)
...
async def list_tasks(
    db: DB, copilot: CopilotDep, task_status: str | None = None
) -> list[TaskSchema]:
    service = ImpactTimelineService(db=db, copilot_client=copilot)
