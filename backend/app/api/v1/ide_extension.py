...
@router.post(
    "/analyze-file", response_model=list[DiagnosticSchema], summary="Analyze file for compliance"
)
async def analyze_file(request: AnalyzeFileRequest, db: DB) -> list[DiagnosticSchema]:
    service = IDEExtensionService(db=db)
    diags = await service.analyze_file(
        file_path=request.file_path, content=request.content, frameworks=request.frameworks or None
    )
    return [
        DiagnosticSchema(
            id=str(d.id),
...
@router.get(
    "/tooltip/{regulation}/{article}",
    response_model=TooltipSchema | None,
    summary="Get regulation tooltip",
)
async def get_tooltip(regulation: str, article: str, db: DB) -> TooltipSchema | None:
    ...
    if not t:
        return None
    return TooltipSchema(
        regulation=t.regulation,
        article=t.article,
        title=t.title,
        summary=t.summary,
        url=t.url,
        obligation_level=t.obligation_level,
    )
...
async def get_posture(file_path: str, db: DB) -> PostureSchema:
    ...
    return PostureSchema(
        ...
        file_score=p.file_score,
        file_grade=p.file_grade,
        violations=p.violations,
        frameworks_affected=p.frameworks_affected,
        repo_score=p.repo_score,
        repo_grade=p.repo_grade,
    )
...
async def get_stats(db: DB) -> ExtStatsSchema:
    service = IDEExtensionService(db=db)
    s = service.get_stats()
    return ExtStatsSchema(
        active_sessions=s.active_sessions,
        diagnostics_shown=s.diagnostics_shown,
        quick_fixes_applied=s.quick_fixes_applied,
        tooltips_displayed=s.tooltips_displayed,
