...
async def join_network(
    request: JoinNetworkRequest, db: DB, copilot: CopilotDep
) -> ParticipantSchema:
    service = ComplianceIntelService(db=db, copilot_client=copilot)
    pl = (
        PrivacyLevel(request.privacy_level)
        if request.privacy_level in [p.value for p in PrivacyLevel]
    ...
    return ParticipantSchema(
        ...
        contributed_patterns=participant.contributed_patterns,
    )
...
async def contribute_pattern(
    request: ContributePatternRequest, db: DB, copilot: CopilotDep
) -> PatternSchema:
    service = ComplianceIntelService(db=db, copilot_client=copilot)
...
async def get_patterns(
    db: DB, copilot: CopilotDep, framework: str | None = None, industry: str | None = None
) -> list[PatternSchema]:
    service = ComplianceIntelService(db=db, copilot_client=copilot)
...
async def similar_orgs(
    db: DB, copilot: CopilotDep, industry: str = "saas", size_category: str = "medium"
) -> list[dict]:
    service = ComplianceIntelService(db=db, copilot_client=copilot)
...
async def get_stats(db: DB, copilot: CopilotDep) -> dict:
    ...
    return {
        ...
        "industries": stats.industries_represented,
        "frameworks": stats.frameworks_covered,
    }
