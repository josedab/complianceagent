...
async def advance(run_id: str, db: DB) -> dict:
    ...
    if not r:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Run not found or cannot advance"
        )
...
async def get_report(run_id: str, db: DB) -> ReportSchema:
    ...
    return ReportSchema(
        run_id=str(rpt.run_id),
        repo=rpt.repo,
        framework=rpt.framework,
        overall_status=rpt.overall_status,
        controls_passed=rpt.controls_passed,
        controls_failed=rpt.controls_failed,
        gaps_resolved=rpt.gaps_resolved,
        gaps_open=rpt.gaps_open,
        evidence_summary=rpt.evidence_summary,
...
async def get_stats(db: DB) -> CertStatsSchema:
    ...
    return CertStatsSchema(
        ...
        in_progress_runs=s.in_progress_runs,
        total_gaps=s.total_gaps,
        resolved_gaps=s.resolved_gaps,
