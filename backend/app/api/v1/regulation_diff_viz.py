...
@router.get(
    "/versions/{regulation}",
    response_model=list[RegVersionSchema],
    summary="List regulation versions",
)
    ...
    return DiffResultSchema(
        ...
        sections=[
            DiffSectionSchema(
                ...
                affected_code_areas=s.affected_code_areas,
...
async def add_annotation(diff_id: UUID, request: AddAnnotationRequest, db: DB) -> AnnotationSchema:
    ...
    return AnnotationSchema(
        id=str(ann.id),
        diff_id=str(ann.diff_id),
        section_id=ann.section_id,
        author=ann.author,
        comment=ann.comment,
        action_required=ann.action_required,
        created_at=ann.created_at.isoformat() if ann.created_at else None,
    )


@router.get(
    "/diff/{diff_id}/annotations", response_model=list[AnnotationSchema], summary="Get annotations"
)
