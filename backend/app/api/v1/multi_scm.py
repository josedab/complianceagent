...
@router.post(
    "/connections",
    response_model=SCMConnectionSchema,
    status_code=status.HTTP_201_CREATED,
    summary="Connect SCM provider",
)
...
@router.post(
    "/sync/{provider}/{organization}",
    response_model=list[UnifiedRepoSchema],
...
async def list_repositories(
    db: DB, provider: str | None = None, limit: int = 50
) -> list[UnifiedRepoSchema]:
    service = MultiSCMService(db=db)
    p = SCMProvider(provider) if provider else None
...
@router.post(
    "/pull-requests",
    response_model=UnifiedPRSchema,
    status_code=status.HTTP_201_CREATED,
    summary="Create compliance PR",
)
async def create_pr(request: CreatePRRequest, db: DB) -> UnifiedPRSchema:
    ...
    return UnifiedPRSchema(
        ...
        compliance_check_status=pr.compliance_check_status,
        url=pr.url,
...
async def process_webhook(request: WebhookRequest, db: DB) -> dict:
    service = MultiSCMService(db=db)
    wh = await service.process_webhook(
        provider=request.provider, event_type=request.event_type, payload=request.payload
    )
