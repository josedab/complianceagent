...
async def create_export(request: CreateExportRequest, db: DB) -> ExportJobSchema:
    ...
    job = await service.create_export(
        data_type=request.data_type, format=request.format, filters=request.filters
    )
...
async def list_exports(
    db: DB, export_status: str | None = None, limit: int = 50
...
async def create_schedule(request: CreateScheduleRequest, db: DB) -> ScheduleSchema:
    ...
    return ScheduleSchema(
        id=str(sched.id),
        name=sched.name,
        data_type=sched.data_type.value,
...
@router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    summary="Configure BI connector",
)
...
async def list_connectors(db: DB) -> list[ConnectorSchema]:
    ...
    return [
        ConnectorSchema(
            connector=c.connector.value, database=c.database, schema_name=c.schema, status=c.status
        )
        for c in conns
    ]
...
async def get_summary(db: DB) -> ExportSummarySchema:
    ...
    return ExportSummarySchema(
        ...
        by_format=s.by_format,
        by_data_type=s.by_data_type,
        total_rows_exported=s.total_rows_exported,
        total_bytes_exported=s.total_bytes_exported,
        active_schedules=s.active_schedules,
        configured_connectors=s.configured_connectors,
    )
