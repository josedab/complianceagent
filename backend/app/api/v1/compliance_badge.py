...
async def get_badge_svg(
    repo: str, db: DB, style: str = "flat", label: str = "compliance"
) -> Response:
    service = ComplianceBadgeService(db=db)
    badge = await service.generate_badge(repo=repo, style=style, label=label)
    return Response(
        content=badge.svg, media_type="image/svg+xml", headers={"Cache-Control": "max-age=300"}
    )
...
@router.get(
    "/scorecard/{repo:path}", response_model=ScorecardSchema, summary="Get public scorecard"
)
...
@router.get(
    "/embed/{repo:path}", response_model=list[EmbedSnippetSchema], summary="Get embed snippets"
)
async def get_embed_snippets(repo: str, db: DB) -> list[EmbedSnippetSchema]:
    service = ComplianceBadgeService(db=db)
    snippets = service.get_embed_snippets(repo)
    return [
        EmbedSnippetSchema(format=s.format, code=s.code, preview_url=s.preview_url)
        for s in snippets
    ]
