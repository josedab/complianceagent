...
class TestIDESemanticAnalysis:
    def test_tooltips_gdpr(self, client):
        r = client.post(
            "/api/v1/ide-agent/semantic/tooltips",
            json={"file_content": "def process_personal_data(u): pass", "file_path": "x.py"},
        )
    ...
    def test_tooltips_critical(self, client):
        r = client.post(
            "/api/v1/ide-agent/semantic/tooltips",
            json={"file_content": "card_number = input()", "file_path": "x.py"},
        )
        assert r.status_code == 200
    ...
    def test_posture_clean(self, client):
        r = client.post(
            "/api/v1/ide-agent/semantic/posture",
            json={"file_content": "x = 1", "file_path": "x.py"},
        )
        assert r.status_code == 200
...
class TestAutoHealing:
    def test_trigger(self, client):
        r = client.post(
            "/api/v1/auto-healing/trigger",
            json={"trigger_type": "scan_violation", "repository": "a/b", "regulation": "gdpr"},
        )
...
class TestRealtimePosture:
    ...
    def test_record_event(self, client):
        r = client.post(
            "/api/v1/realtime-posture/events",
            json={
    ...
    def test_alert_rule_crud(self, client):
        r = client.post(
            "/api/v1/realtime-posture/alert-rules",
            json={"name": "x", "metric": "score", "threshold": 80.0, "channel": "slack"},
        )
        assert r.status_code in (200, 201)
...
class TestRegSimulator:
    ...
    def test_create_scenario(self, client):
        r = client.post(
            "/api/v1/reg-simulator/scenarios",
            json={
                "regulation": "gdpr",
                "change_description": "New consent rules",
                "affected_articles": ["Art 6"],
            },
        )
        assert r.status_code in (200, 201)
...
class TestIaCPolicy:
    def test_scan_terraform(self, client):
        r = client.post(
            "/api/v1/iac-policy/scan/terraform",
            json={"file_content": "resource aws_s3_bucket b {}", "file_path": "main.tf"},
        )
...
class TestNetwork:
    def test_join(self, client):
        r = client.post(
            "/api/v1/compliance-network/join", json={"industry": "fintech", "size_bucket": "medium"}
        )
        assert r.status_code in (200, 201)
